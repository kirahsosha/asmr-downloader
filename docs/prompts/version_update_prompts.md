...
> 涉及版本号升级、版本口径同步或 `docs/wpf-migration-progress.md` 的版本维护时，**必须**完整读取并严格遵守以下全部规则。
...
1. **建立 todo list**：按 `docs/prompts/global_prompts.md` 的 `§1.2` 要求。
2. **更新版本号**：
...
3. **同步 progress 文档**（按顺序）：
   ...
   - 在 `1.5` 追加新记录，不修改旧记录。
   - 在 `2.1` 更新当前测试样例基线。
   - 在 `3.1` 新增或合并单一待提交行（见 `docs/prompts/document_special_prompts.md` 的 `§1.4` 与 `docs/prompts/submission_policy_prompts.md` 的 `§1`）。
   - 在 `4` 将受影响项重置为未勾选，并新增必要验证项（见 `docs/prompts/document_special_prompts.md` 的 `§1.5`）。
4. **验证**（建议顺序）：
   - `Application.Tests`
...
- [ ] 未误改 `bin/`、`obj/` 产物。
- [ ] 未误动历史 Go 版本口径。
- [ ] 未读取或修改被排除的文件（见 `docs/prompts/global_prompts.md` 的 `§1.3` 与 `docs/prompts/document_special_prompts.md` 的 `§1.1`）。
